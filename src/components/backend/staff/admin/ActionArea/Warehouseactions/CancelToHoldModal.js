...
const CancelToHoldModal=(props)=>{
    ...
    const toggle = props.toggle;
     ...
     const[cancelToHoldPickupId,setCancelToHoldPickupId]=useState('');
     const [cancelToHoldData, setCancelToHoldData] = useState({
         pickup_id:'',
         remarks:''
     ...
     useEffect( ()=>{
        ...
        console.log(props);
         getCancelToHoldPickupId(props.packetId);
         console.log(cancelToHoldPickupId);

    },[props.packetId]);
      const getCancelToHoldPickupId=(data)=>{
       setCancelToHoldPickupId(data);
    }

   const selectChange = event => {
       if(event){
            const field = {...cancelToHoldData};
            field.remarks = event.target.value;
             field.pickup_id = cancelToHoldPickupId;
            setCancelToHoldData(field);
    ...
    const getAllReturnAndCancel = () => {
     ...
     const onSubmit = async (event) => {
         ...
         console.log(cancelToHoldPickupId);
          console.log(cancelToHoldData);
         axios.post('/admin/pickup/move/to/hold' ,cancelToHoldData)
             .then((res) => {
                 console.log(res);
                 ...
                 <Modal.Body style={{width:'400px'}}>
                     ...
                     <Form onSubmit={handleSubmit(onSubmit)} ref={form => formRef = form}>
                          ...
                          <Row>
                             <Col lg={6}>
                                <Form.Group className="mt-0">
                                      <Button type="submit" variant="primary" style={{marginTop:'35px',width:'100%'}}>Move Hold</Button>
                                </Form.Group>
                             </Col>
                             <Col lg={6}>
                                 <Form.Group className="mt-0">
...
export default CancelToHoldModal