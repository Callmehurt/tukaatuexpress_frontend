...
const EntryImagePickupCreate=()=>{
     ...
     const[requestImageData,setRequestImageData]=useState('');
     const[rotateImage,setRotateImage]=useState(0);
     const [currentCustomer, setCurrentCustomer] = useState('');
     const [formField, setFormField] = useState({
        customer_id: '',
        partner_id: location.state.entryImageData.partner_id,
        type: '',
        weight:'',
        packet_name: '',
        cod: '',
        ...
        request_id:location.state.entryImageData.request_id,
        image_id:location.state.entryImageData.image_id,
        received_status:1,
    ...
    useEffect(()=>{
        ...
        // console(location.state.entryImageData);
        getRequiredDataForEntry();
        loadCustomer();
    ...
    const FindFormErrors = () =>{
     ...
     const [formFieldCustomer, setFormFieldCustomer] = useState({
      ...
      const loadCustomer = async () => {
        await axios.get('/entry_operator/customer/list')
            .then((res) => {
                console.log(res);
                let cusArr =[]
                res.data.map((data) => {
    ...
    const getRequiredDataForEntry=()=>{
        setRequestImage(location.state.entryImage);
        setRequestImageData(location.state.entryImageData);
        let img_id=location.state.entryImageData.image_id;
        axios.get(`/entry_operator/partner/pickup/request/detail/${img_id}`)
                .then((res) => {
                    console.log(res);
                    const newField={...formField};
                    newField.partner_id=res.data.request_detail?.partner_id;
                    setFormField(newField);
    ...
    const onSubmit = async (event) => {
        ...
        if ( Object.keys(newErrors).length > 0 ) {
         ...
         await axios.post('/entry_operator/partner/proceed/pickup/request', formField)
            .then((res) => {
                console.log(res)
                if(res.data.status === true){
                    ...
                    setLoading(false);
                    history.push({
                        pathname:'/entry_operation/pickup_request',
         ...
         if ( Object.keys(newErrors).length > 0 ) {
          // We got errors!
           setFormerrors(newErrors);
            ...
            axios.post('/entry_operator/customer/register', formFieldCustomer)
                .then((res) => {
                    console.log(res);
      ...
      {/*>*/}
                ...
                <Col lg={6} className="pt-3">
                    <div style={{position:'relative',display:'flex'}}>
                       <img src={requestImage} style={{transform:`rotate(${rotateImage}deg)`}} className="img-fluid" />
...
export default EntryImagePickupCreate