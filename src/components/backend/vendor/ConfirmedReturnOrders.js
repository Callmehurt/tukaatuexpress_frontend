...
import {returnsOrderList,returnsConfirmsOrderList} from "../../../redux/actions/vendor";

const ConfirmedReturnOrders=()=>{
    const history = useHistory();
     ...
     useEffect(()=>{
         let vendorDetail = JSON.parse(localStorage.getItem('vendorDetail'));
            ...
            getConfirmReturnList();



     },[]);
     const getConfirmReturnList=()=>{
            axios.get('/partner/get/cancelled/confirm/pickups')
                    .then((res)=>{
                        console.log(res);
                        console.log(res.data);
                        dispatch(returnsConfirmsOrderList(res.data));
                    })
            .catch((err)=>{
    ...
    return(
        <>
             <div style={{height:'80vh',overflowY:'auto',overflowX:'hidden'}}>
                    <div>
                        {returnsConfirmsOrdersList.length?<>
                            {
                                returnsConfirmsOrdersList.map((list)=>(
                                    <>
                                        <Col lg={12}>
...
export default ConfirmedReturnOrders