...
const MarketingStaffTable = () => {
    const marketingStaff = useSelector((state) => state.monthlyStatementDetails.marketingStaffs);
  const renderList = marketingStaff.map((staff) => {
    return (
  ...
  return (
    <TableContainer component={Paper}>
        <h5 style={{fontSize:'1rem'}}>Marketing Staff</h5>
      <Table sx={{ minWidth: 600 }} aria-label="customized table">
        ...
        <TableBody>
           <>
           {Object.keys(marketingStaff).length === 0 ? (
                    <StyledTableRow>
...
export default MarketingStaffTable;