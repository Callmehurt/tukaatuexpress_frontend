...
import {marketingLogout} from "../../../redux/actions/Marketing";
...
const MarketingHeader=()=>{
    const location = useLocation();
     ...
     const adminStaffName=marketingAuth.user;
        ...
        if ( Object.keys(newErrors).length > 0 ) {
          // We got errors!
           setFormerrors(newErrors);
             ...
             // console.log("formField");
            axios.post('/marketing/change/password', formField)
            .then((res) => {
                console.log(res)
     ...
     const logoutAdmin=()=>{
        console.log('logout');
         ...
         let marketingAdmin = JSON.parse(localStorage.getItem('marketingAdmin'));
        if(marketingAdmin){
             localStorage.removeItem('marketingAdmin');
             dispatch(marketingLogout(''));
